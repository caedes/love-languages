...
export default class App extends React.Component {
  ...
  renderHome() {
    ...
    return (
      <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 20, animation: 'lqFade .3s ease' }}>
        <div>
          <p style={{ fontSize: 11, letterSpacing: '.14em', textTransform: 'uppercase', color: muted(50), margin: '0 0 10px' }}>Questionnaire à choix forcés</p>
          <h1 style={{ fontSize: 'clamp(26px, 7.5vw, 34px)', lineHeight: 1.08, letterSpacing: '-.02em', margin: '0 0 10px' }}>Les 5 langages de l'amour</h1>
          <p style={{ fontSize: 14, color: muted(75), margin: 0 }}>30 paires, une par écran. Vous répondez tous les deux sur le même appareil, puis les profils sont comparés. Vos réponses restent sur cet appareil, rien n'est envoyé.</p>
        </div>

        <div style={{ display: 'grid', gap: 12 }}>
          {consignes.map((t, i) => (
            <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
              <span style={{ flex: 'none', width: 22, height: 22, borderRadius: '50%', border: '1px solid var(--color-accent-600)', color: 'var(--color-accent-300)', display: 'grid', placeItems: 'center', fontSize: 11 }}>{i + 1}</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div className="field">
            ...
            <input className="input" id="lq-a" type="text" autoComplete="off" placeholder="Prénom" value={st.nameA} onChange={(e) => this.setState({ nameA: e.target.value })} style={{ minHeight: 46, fontSize: 16 }} />
          </div>
          <div className="field">
            <label htmlFor="lq-b">Second prénom</label>
            <input className="input" id="lq-b" type="text" autoComplete="off" placeholder="Prénom" value={st.nameB} onChange={(e) => this.setState({ nameB: e.target.value })} style={{ minHeight: 46, fontSize: 16 }} />
          </div>
        </div>
  ...
  renderResults() {
    ...
    return (
      <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', gap: 12, animation: 'lqFade .3s ease' }}>
        <div>
        ...
        <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
            ...
            <div style={{ display: 'grid', gap: 12 }}>
              {[0, 1].map((who) => (
                <div key={who} style={{ background: 'var(--color-surface)', border: '1px solid var(--color-divider)', borderRadius: 'var(--radius-lg)', padding: '14px 16px 12px' }}>
                    ...
                    {scoring.profileRows(this.state.answers[who]).map((d) => (
                      <div key={d.code}>
                        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 10 }}>
                          ...
                          <span style={{ fontSize: 13, fontVariantNumeric: 'tabular-nums', color: muted(68) }}>{d.score}/12</span>
                        </div>
                        <div style={{ height: 7, borderRadius: 4, background: muted(10), overflow: 'hidden', margin: '6px 0 4px' }}>
                  ...
                  <p style={{ margin: '12px 0 0', paddingTop: 9, borderTop: '1px solid var(--color-divider)', fontSize: 11.5, color: muted(55) }}>
          ...
          {st.tab === 'vigilance' && (
            <div style={{ border: '1px solid var(--color-accent-700)', background: 'linear-gradient(180deg, color-mix(in srgb, var(--color-accent) 12%, transparent), color-mix(in srgb, var(--color-accent) 4%, transparent))', borderRadius: 'var(--radius-lg)', padding: 16 }}>
              <p style={{ fontSize: 12, color: muted(60), margin: '0 0 14px' }}>Dimensions où l'un est en langage primaire et l'autre en canal neutre.</p>
              {vig.length === 0
                ? <p style={{ margin: 0, fontSize: 14, color: muted(80) }}>Aucun écart de ce type : personne n'a de langage primaire là où l'autre a un canal neutre.</p>
                : (
                  <div style={{ display: 'grid', gap: 12 }}>
                    {vig.map((v, i) => (
                      <p key={i} style={{ margin: 0, paddingLeft: 12, borderLeft: `3px solid ${v.color}`, fontSize: 14, textWrap: 'pretty' }}>
                        <strong style={{ fontWeight: 600 }}>{v.strong}</strong> a un besoin fort de <em>{v.dim}</em>, une dimension peu sensible chez <strong style={{ fontWeight: 600 }}>{v.weak}</strong>.
                      </p>
                    ))}
                  </div>
                )}
          ...
          {st.tab === 'divergences' && (
            dc ? (
              <div>
                <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-divider)', borderRadius: 'var(--radius-lg)', padding: '14px 16px' }}>
                  <p style={{ margin: '0 0 12px', fontSize: 11, letterSpacing: '.1em', textTransform: 'uppercase', color: muted(45) }}>Item {dc.id}</p>
                  <div style={{ display: 'grid', gap: 14 }}>
                    <div style={{ paddingLeft: 12, borderLeft: `3px solid ${dc.colorA}` }}>
                      <p style={{ margin: '0 0 4px', fontSize: 12.5, color: dc.colorA }}>{dc.nameA} · {dc.dimA}</p>
                      <p style={{ margin: 0, fontSize: 14.5, textWrap: 'pretty' }}>{dc.textA}</p>
                    </div>
                    <div style={{ paddingLeft: 12, borderLeft: `3px solid ${dc.colorB}` }}>
                      <p style={{ margin: '0 0 4px', fontSize: 12.5, color: dc.colorB }}>{dc.nameB} · {dc.dimB}</p>
                      <p style={{ margin: 0, fontSize: 14.5, textWrap: 'pretty' }}>{dc.textB}</p>
                    </div>
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginTop: 10 }}>
                  <button className="btn btn-secondary" onClick={() => this.setState({ divIdx: (st.divIdx - 1 + div.length) % div.length })} style={{ minHeight: 40, fontSize: 13 }}>‹ Précédent</button>
                  <span style={{ fontSize: 12.5, fontVariantNumeric: 'tabular-nums', color: muted(55) }}>{Math.min(st.divIdx, div.length - 1) + 1} / {div.length}</span>
                  <button className="btn btn-secondary" onClick={() => this.setState({ divIdx: (st.divIdx + 1) % div.length })} style={{ minHeight: 40, fontSize: 13 }}>Suivant ›</button>
                </div>
              </div>
            ) : (
              <p style={{ margin: 0, fontSize: 14, color: muted(80) }}>Aucune divergence : vous avez choisi la même proposition sur les 30 items.</p>
            )
          )}
        </div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button className="btn btn-secondary" onClick={() => this.setState({ screen: 'home', copied: false, copyText: null })} style={{ flex: 1, minHeight: 44, fontSize: 13 }}>Recommencer</button>
        </div>
      </div>
    );
  }
  ...
  render() {
    ...
    return (
      <div style={{ height: '100dvh', overflow: 'hidden', background: 'radial-gradient(120% 70% at 50% -10%, #1c1f31 0%, var(--color-bg) 60%)', fontFamily: 'var(--font-body)', fontSize: 16, lineHeight: 1.5, padding: '16px 16px 14px', display: 'flex' }}>
        <div style={{ width: '100%', maxWidth: 540, margin: '0 auto', minHeight: 0, display: 'flex', flexDirection: 'column' }}>
